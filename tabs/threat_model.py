...
# Function to get the list of installed models from LMstudio
def get_installed_models(lmstudio_url):
    response = requests.get(f"{lmstudio_url}/v1/models/")
    if response.status_code == 200:
        models = response.json()
        print("Models response:", models)  # Debugging line
        return models.get("data", [])
    else:
        return []

# Function to generate text using the selected model
def generate_threat_model_llmlocal(model_name, messages, lmstudio_url):
    data = {
        "model": model_name,
        "messages": messages,
        "temperature": 0.7,
        "max_tokens": 6096,  # Adjust as needed
        "stream": False,
    ...
    if response.status_code == 200:
        ...
        print("Generate text response:", response_json)  # Debugging line
        return response_json.get("choices", [{}])[0].get("message", {}).get("content", "")
    else:
        return ""

# Function to parse and format the LLM response into the desired structure
def get_threat_model_llmlocal(lmstudio_url, model_name, prompt):
    messages = [
        {"role": "system", "content": "You are a helpful assistant. Please provide a response in strict JSON format."},
        {"role": "user", "content": prompt}
    ]

    # Generate the threat model using the selected model (local LLM)
    generated_text = generate_threat_model_llmlocal(model_name, messages, lmstudio_url)
    ...
    if generated_text:
        try:
            ...
            if json_start != -1 and json_end != -1:
                ...
                model_output = json.loads(json_content)

                # Ensure the output is in the desired format (comma-separated Threats and Potential Consequences)
                formatted_output = []
                for entry in model_output.get("threat_model", []):
                    # Convert 'Threats' and 'Potential Consequences' from arrays to comma-separated strings if they are lists
                    if isinstance(entry.get("Threats"), list):
                        entry["Threats"] = ', '.join(entry["Threats"])
                    if isinstance(entry.get("Potential Consequences"), list):
                        entry["Potential Consequences"] = ', '.join(entry["Potential Consequences"])
                    
                    # Add the formatted entry to the final output list
            ...
            print(f"Failed to parse the generated JSON response: {e}")
            print("Raw output received:")
            print(generated_text)  # Print the raw output for debugging
            formatted_output = []
    else:
        # Handle case where no text is generated by the local model
        print("Failed to generate the threat model. Please check the model and try again.")
        formatted_output = []

    return {"threat_model": formatted_output}